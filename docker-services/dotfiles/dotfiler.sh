...
Pkglist file:
  ...
  If a pkglist file ends with a bang ('!', e.g. "pkglist-i3!") the packages it
  tracks will be exclusive to that system and won't be tracked by any other
  systems. Keep in mind that you'll have to manually insert the packages in the
  file for it to track them. This is intended as a feature to help users track
  packages that are used only with a very specific setup.
...
CAVEATS:
  This program only keeps track of your packages with "pacman", "paru" and "yay"
  on Arch or "apt" on Ubuntu. Merge request the project to add support for
  other managers.
...
ChoosePkgMgr ()
{
	manager="$(FindManager "apt" "dnf" "paru" "yay" "pacman")"
...
# == FILTER BANG PACKAGES ==
#
# Searches all pkglist files and removes the packages from the files ending with
# a bang from the current system's packages.
#
# == Args ==
...
FilterBangPkgs ()
{
	...
	if [ "$(find . -name "pkglist-*\!" | wc -l)" -gt 0 ]
	then
		for i in pkglist-*!
		do
			sed -i \
...
Deploy ()
{
	...
	if [ "$1" = "DOTFILER-DEFAULT-SYSTEM" ]
	then
		sysname="system"
		systag=""
	fi

	if Confirm "yes" "Deploy dotfiles from \"$sysname\" to system"
	then
...
Install ()
{
	...
	then
		sysname="system"
		systag=""
	elif [ "$(find . -name "pkglist$systag*" | sed 's/.\+\(.$\)/\1/')" == "!" ]
	then
		systag="$systag!"
	fi


	if Confirm "yes" "Review packages list (it will be permanently modified)"
	then
	...
	if Confirm "no" "Proceed with the $sysname packages installation"
	then
...
InstallParu ()
{
	pacman -Q paru 1>/dev/null 2>&1 || {
		git --version 1>/dev/null 2>&1 || sudo pacman --noconfirm -S git
		rm -rf paru 1>/dev/null 2>&1
		git clone https://aur.archlinux.org/paru.git
		cd paru && makepkg --noconfirm -si && cd ..
		rm -rf paru
	}

	paru --noconfirm --cleanafter --skipreview --needed -S - < "pkglist$1"
}
...
Pull ()
{
	...
	if Confirm "yes" "Pull dotfiles from \"$sysname\" to repository"
	then
		printf "\033[1;35m==> \033[0;1mProcessing \"%s\" files\n" "$sysname"
...
Update ()
{
	...
	if Confirm "yes" "Update \"$sysname\" package list"
	then
		ChoosePkgMgr "Update" "$systag"
	fi

	FilterBangPkgs "$systag"
}
...
if [ "$systems" = "" ]
then
	systems="DOTFILER-DEFAULT-SYSTEM"
fi
